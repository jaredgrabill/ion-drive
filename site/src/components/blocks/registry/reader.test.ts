/**
 * Parity tests for the site's vendored protocol reader (spec-10 AC4) — a
 * port of `packages/cli/src/registry/protocol.test.ts`: the schemaVersion
 * format gate (with spec-01's exact legacy-index message), the load-bearing
 * field + cross-field checks, unknown-field tolerance, the URL helpers, and
 * the **parity check**: a fully strict fixture accepted by core's
 * `parseRegistryIndex`/`parseRegistryBlock` must be accepted by this reader
 * too. Plus the spec-08 optional fields (`searchUrl`/`registriesUrl`/
 * `readmeUrl`) passing through.
 */

// Core is a devDependency — test-only import (the CLI's exact precedent).
...
describe('lenient reading', () => {
  ...
  it('passes the spec-08 optional fields through', () => {
    const index = parseIndexDoc(
      { ...strictIndex, searchUrl: 'search-index.json', registriesUrl: '../registries.json' },
      URL_OF,
    );
    expect(index.searchUrl).toBe('search-index.json');
    expect(index.registriesUrl).toBe('../registries.json');

    const block = parseBlockDoc({ ...strictBlock, readmeUrl: 'crm.readme.md' }, URL_OF);
    expect(block.readmeUrl).toBe('crm.readme.md');
  });
  ...
  it('parses a registries directory leniently, skipping bad entries', () => {
    const parsed = parseRegistriesDirectoryDoc(
      {
        schemaVersion: 1,
        registries: [
          { namespace: '@ion', url: 'https://registry.iondrive.dev/registry/index.json' },
          { namespace: 42 }, // skipped, not fatal
          'garbage',
        ],
      },
      URL_OF,
    ...
    expect(parsed.registries[0]?.namespace).toBe('@ion');
    expect(() => parseRegistriesDirectoryDoc({ registries: [] }, URL_OF)).toThrow(
