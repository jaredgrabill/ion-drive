...
| 2026-07-14 | **Discovery: F23 was already half-unblocked** | The watcher's baseline showed pre-existing state the plan's premise missed: `v0.4.0` was tagged and published **2026-07-10** from `f0b948e` — **before** the V1–V7 security fixes (merged 07-12). Live now: all 7 packages on npm (fixed group 0.4.0, plugins 0.1.0); blocks repo pushed (tag `v0.2.0`, no `v1`); `registry.iondrive.dev` **and** `iondrive.dev` live; GHCR 0.4.0 missing or private (`no such manifest` anonymously; token lacks read:packages). **Verified live: `npx @ion-drive/cli@0.4.0 init` scaffolds `.env` without `ION_REQUIRE_AUTH` — the published CLI ships the V1 insecure default.** |
| 2026-07-14 | Security republish staged (0.4.1) | Changeset consumed, fixed group bumped to **0.4.1** on main (cef7514) with a CHANGELOG entry naming the V1–V7 gap. Owner tags `v0.4.1` to publish. Recommend also `npm deprecate` on the four 0.4.0 packages after 0.4.1 lands. |
| 2026-07-14 | Lane 1 §4 [AGENT] verify | Registry matrix 8/8: index schemaVersion 1, crm latest 0.2.0, registries.json `@ion`, schema `$id`, search-index (5 docs), crm artifact HEAD 200, badge SVG, readme byte copy. Site 5/5: landing, `/docs/getting-started/`, `/blocks/`, `/schemas/*.v1.json` 200; `/blocks/crm` deep link returns HTTP 404 **whose body is the SPA fallback with the BlocksBrowser island** (GitHub Pages fallback semantics — as designed, spec-10). |
| 2026-07-14 | Lane 1 §6 (partial smokes) | Published CLI against the live registry: `search invoicing` → 1 hit "via search index", tier `official (claimed)` (claimed-not-attested is correct pre-attested-publish); `registry add @probe` → the exact not-in-directory hint. Remaining §6 gated on the blocks-repo push (audit@0.1.1 is the new artifact that exercises the attest step — the agent's `git push` to ion-drive-blocks was permission-denied, so it moved to the owner queue). |
| 2026-07-14 | **Lane 1 exit criterion MET (stranger flow, published artifacts only)** | On a clean scratch dir with zero access to our machines: `npx @ion-drive/cli@0.4.0 init` → `npm install` from public npm (549 pkgs, **42.7s**) → `npm run dev` (**5.6s** to `/health` 200) → `/admin` 200 → anonymous MCP `tools/list` = **17 tools** → `npx ion-drive add crm` resolved via `registry.iondrive.dev`, digest-verified `sha256:a5db8b9f…`, tier `○ community (unattested)` (correct pre-attestation), vendored code hot-reloaded and all 3 actions + hook registered — **8.5s**. Tool-time total ≈90s; the 5-minute promise holds with room for the human steps. Caveats: runs on the **insecure 0.4.0** (auth off — hence anonymous MCP working), and tier upgrades to ◆ official only after the attested blocks publish. |
| 2026-07-14 | Lane 1 §6 — spec-06 AC6 dogfood loop | With **published** packages only (`npm i @ion-drive/cli @ion-drive/core` from the registry): `ion-drive block new demo` → `block validate` ✔ → `block pack` ✔ (repack **byte-identical**) → `block test` **7/7** (ephemeral server on scratch DB, block-local `test/` 2/2, uninstall residue-free). |
| 2026-07-14 | **v0.4.1 published + security verified on published artifacts** (owner authorized tags/pushes in chat) | Tag `v0.4.1` → Release run green end-to-end (npm ×4 + GHCR — this real run supersedes the §3 dry-run: every guard and publish step executed for real). Verified on the **published** artifacts: `npm view` 0.4.1 ×4; scaffold `.env` has `ION_REQUIRE_AUTH=true`; prod+auth-off boot refuses (V1 message verbatim); `ION_CORS_ORIGINS=*` refuses (V2); signup 200→**403** with lockout on; authenticated MCP agent loop passes (17 tools, serverInfo reports 0.4.1); `ghcr.io/jaredgrabill/ion-drive:0.4.1` pulls **publicly**, boots, `/health`+`/admin` 200. Definition-of-Done #2 satisfied. |
| 2026-07-14 | **First attested publish (Lane 1 §5 / spec-05 AC4)** | audit@0.1.2 as a source-only bump → publish run packs, attests, commits `block.json.sigstore.json` (learning codified: a pre-packed+registered artifact can't be re-attested — immutability guard — so exercise the loop with a new version, source-only). `gh attestation verify` exit 0; `ion-drive block verify audit@0.1.2` → attestation OK, tier **◆ official**; from a scratch 0.4.1 project `ion-drive add audit` → `◆ official … (attested: jaredgrabill/ion-drive-blocks@e6f8ef9)`; live registry serves latest 0.1.2. Incidental fixes: publish workflow no longer sweeps runner-local build reports into publish commits (1404a76); stray `finalize.json` removed. |
