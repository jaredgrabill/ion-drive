...
export function createDepthLimitRule(maxDepth: number) {
  return (context: ValidationContext): ASTVisitor => {
    const measure = (
      ...
      for (const selection of set.selections) {
        ...
        if (selection.kind === Kind.FIELD) {
          ...
          child = selection.selectionSet
            ? measure(selection.selectionSet, depth + 1, seenFragments)
            : depth;
        } else if (selection.kind === Kind.INLINE_FRAGMENT) {
          child = measure(selection.selectionSet, depth, seenFragments);
        } else {
          ...
          if (fragment) {
            child = measure(fragment.selectionSet, depth, new Set([...seenFragments, name]));
          }
        }
    ...
    return {
      OperationDefinition: {
        leave(node) {
          const depth = measure(node.selectionSet, 1, new Set());
          if (depth > maxDepth) {
            context.reportError(
              new GraphQLError(
