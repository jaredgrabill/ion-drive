...
export const registryIndexSchema = z
  .object({
    ...
    /**
     * URL of the registry's prebuilt search index (spec-08), relative to the
     * index or absolute https. Optional and backward-compatible: consumers
     * without it fall back to substring search over `blocks`.
     */
    searchUrl: z.string().min(1).optional(),
    /**
     * URL of the registry's `registries.json` directory (spec-08), relative to
     * the index or absolute https. Optional: the shipped Pages layout serves
     * the directory at the repo root (not beside `index.json`), so clients
     * without this field fall back to probing the index's sibling.
     */
    registriesUrl: z.string().min(1).optional(),
...
export const registryBlockSchema = z
  .object({
    ...
    /**
     * URL of the block's copied README (spec-08), relative to this file or
     * absolute https. Mutable display data (NOT a per-version release
     * artifact) — regenerated by `registry build` from the source README.
     */
    readmeUrl: z.string().min(1).optional(),
