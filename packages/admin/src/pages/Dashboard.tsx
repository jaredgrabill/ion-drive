...
export function Dashboard() {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-4">
      ...
      <StatusBanner status={status} version={version.data} />
      ...
      {/* Charts row */}
      <ChartsRow loading={traffic.isLoading} traffic={traffic.data} />

      {/* Recent errors + recent objects */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <RecentErrorsCard errors={errors.data ?? []} />
        <RecentObjectsCard userObjects={userObjects} />
      </div>

      {/* Installed blocks */}
      <BlocksCard blocks={blocks.data ?? []} />
    </div>
  );
}
...
/** Health banner: status dot + headline + version/uptime when known. */
function StatusBanner({
  status,
  version,
}: {
  status: ReturnType<typeof useHealth>['status'];
  version: { version: string; uptimeSeconds: number } | undefined;
}) {
      ...
      <CardContent className="flex flex-wrap items-center gap-3 py-3">
        ...
        {version && (
          <p className="ml-auto text-sm text-muted-foreground">
            v{version.version} · up {formatUptime(version.uptimeSeconds)}
          </p>
        )}
      </CardContent>
    </Card>
...
/** Traffic + latency chart cards (skeletons while loading, hint when empty). */
function ChartsRow({
  loading,
  traffic,
}: {
  loading: boolean;
  traffic: Awaited<ReturnType<typeof api.traffic>> | undefined;
}) {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
      <div className="lg:col-span-3">
        <ChartCard title="API Traffic (24h)">
          ...
          ) : (traffic?.totals.requests ?? 0) === 0 ? (
            <div className="flex h-[240px] items-center justify-center text-sm text-muted-foreground">
              No traffic recorded yet.
            </div>
          ) : (
            traffic && (
              <TrafficAreaChart points={traffic.points} bucketMinutes={traffic.bucketMinutes} />
            )
          )}
        </ChartCard>
      </div>
      <div className="lg:col-span-2">
        <ChartCard title="Response Times (24h)">
          {loading ? (
            ...
            traffic && <LatencyBarChart latency={traffic.latency} />
          )}
        </ChartCard>
      </div>
    </div>
...
/** Latest error responses with status badges, linking to the Logs page. */
function RecentErrorsCard({ errors }: { errors: Awaited<ReturnType<typeof api.recentErrors>> }) {
  return (
    <Card>
        ...
        {errors.length === 0 ? (
          ...
          <ul className="flex flex-col divide-y divide-border/60">
            {errors.map((err) => (
              ...
              >
                <Badge variant={err.status >= 500 ? 'destructive' : 'warning'}>{err.status}</Badge>
                <span className="font-mono text-xs">{err.method}</span>
                <span className="truncate font-mono text-xs text-muted-foreground">{err.path}</span>
...
RecentErrorsCard.displayName = 'RecentErrorsCard';

/** The six most recent user-defined objects, linking to their detail pages. */
function RecentObjectsCard({
  userObjects,
}: {
  userObjects: Awaited<ReturnType<typeof api.listObjects>>;
}) {
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between p-3">
...
RecentObjectsCard.displayName = 'RecentObjectsCard';

/** Installed building blocks summary row (hidden when none are installed). */
function BlocksCard({ blocks }: { blocks: Awaited<ReturnType<typeof api.listBlocks>> }) {
...
BlocksCard.displayName = 'BlocksCard';
